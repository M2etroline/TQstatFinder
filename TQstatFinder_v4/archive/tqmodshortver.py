...
def bonus_digester(bonus_list,item,name,percent,sortby):
    for i in bonus_list:
        try:
            if 'Chance' in i:
                if len(i.split())==len(name.split())+2:
                    if set(name.split()).issubset(i.split()):
                        if percent == 1:
                            if '%' in i.split()[3]:
                                if sortby == 'chance':
                                    return [int(i.split()[0][:-3]),i,item]
                                else:
                                    if '+' in i.split()[3]:
                                        return [int(i.split()[3][1:-1]),i,item]
                                    elif '-' in i.split()[3]:
                                        return [-int(i.split()[3][1:-1]),i,item]
                                    else:
                                        return [int(i.split()[3][:-1]),i,item]
                        else:
                            if '%' not in i.split()[3]:
                                return [int(i.split()[3][1:]),i,item]
            else:
                if len(i.split())==len(name.split())+1:
                    if set(name.split()).issubset(i.split()):       
                        if percent:
                            if '%' in i:
                                if '+' in i:
                                    return [int(i.split()[0][1:-1]),i,item]
                                elif '-' in i:
                                    return [-int(i.split()[0][1:-1]),i,item]
                                else:
                                    return [int(i.split()[0][:-1]),i,item]
                        else:
                            if '%' not in i:
                                if '+' in i:
                                    return [int(i.split()[0][1:]),i,item]
...
Check('Energy Regeneration per second',0,'value')       
